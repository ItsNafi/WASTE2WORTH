...
const ScrapModel = {
  ...
  async create({ ownerId, category, weight, photoUrl, latitude, longitude }) {
    const [result] = await pool.execute(
      `INSERT INTO ScrapListings
         (ownerId, category, weight, photoUrl, latitude, longitude)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [ownerId, category, weight, photoUrl || null, latitude, longitude]
    );
    return {
  ...
  async findAll() {
    const [rows] = await pool.execute(
      `SELECT s.listingId, s.ownerId, s.category, s.weight, s.status,
              s.photoUrl, s.createdAt, u.name AS ownerName
