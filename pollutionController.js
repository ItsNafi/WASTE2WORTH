...
const PollutionController = {
  async submitComplaint(req, res) {
    try {
      ...
      let coordinates;
      try {
        coordinates = parseOptionalCoordinates(req.body.latitude, req.body.longitude);
      } catch (coordinateError) {
        return res.status(coordinateError.status || 400).json({ error: coordinateError.message });
      }

      const photoUrl = req.file ? '/uploads/' + req.file.filename : null;

      await PollutionModel.create({
        citizenId, locationPin, description, photoUrl, ...coordinates
      });
