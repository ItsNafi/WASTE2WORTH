...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (currentPath.includes('/admin')) {
    ...
    const campaignStatusLabel = (status) => status === 'Active' ? 'Ongoing' : status;

    const resetCampaignForm = () => {
      const form = document.getElementById('adminCampaignForm');
      if (!form) return;

      form.reset();
      document.getElementById('campaignId').value = '';
      document.getElementById('campaignCap').value = '50';
      document.getElementById('campaignFormTitle').textContent = 'Launch New Campaign';
      document.getElementById('campaignSubmitBtn').textContent = 'Launch Campaign';
      document.getElementById('cancelCampaignEdit').classList.add('hidden');
    ...
    const loadAdminCampaigns = async () => {
      const tbody = document.getElementById('campaignAdminTbody');
      if (!tbody) return;

      const params = new URLSearchParams();
      const search = document.getElementById('campaignSearch')?.value.trim();
      if (search) params.set('search', search);
      if (activeCampaignStatus) params.set('status', activeCampaignStatus);

      tbody.innerHTML = `<tr><td colspan="8" class="text-center"><div class="spinner"></div></td></tr>`;

      try {
        const query = params.toString();
        const campaigns = await apiCall(`/api/admin/campaigns${query ? `?${query}` : ''}`);

        if (campaigns.length === 0) {
          tbody.innerHTML = `<tr><td colspan="8" class="text-center text-muted" style="padding:40px;">No campaigns found.</td></tr>`;
          return;
        }
        ...
        tbody.innerHTML = campaigns.map(campaign => `
          <tr>
            <td>
              ${campaign.imageUrl
                ? `<img src="${campaign.imageUrl}" class="campaign-table-image" alt="${escapeHTML(campaign.title)}">`
                : `<div class="campaign-table-image campaign-image-placeholder"><span class="material-icons-outlined">image</span></div>`
              }
            </td>
            <td style="font-weight:600;">${escapeHTML(campaign.title)}</td>
            <td>${escapeHTML(campaign.boundaryZone)}</td>
            <td>${formatDate(campaign.date)}</td>
            <td>${escapeHTML(campaign.organizerName || '')}</td>
            <td>${campaign.currentVolunteers}/${campaign.participantCap}</td>
            <td><span class="status-pill" data-status="${campaign.status}">${campaignStatusLabel(campaign.status)}</span></td>
            <td>
              <div class="campaign-actions">
                <button type="button" class="btn btn-outline btn-sm" onclick="editCampaign(${campaign.campaignId})">Edit</button>
                <button type="button" class="btn btn-danger btn-sm" onclick="deleteCampaign(${campaign.campaignId})">Delete</button>
              </div>
            </td>
          </tr>
        `).join('');
      } catch (err) {
        tbody.innerHTML = `<tr><td colspan="8" class="text-center form-error">Failed to load campaigns.</td></tr>`;
      }
    };
    ...
    window.editCampaign = async (campaignId) => {
      try {
        const campaign = await apiCall(`/api/admin/campaigns/${campaignId}`);
        ...
        document.getElementById('campaignZone').value = campaign.boundaryZone || '';
        document.getElementById('campaignDate').value = formatDateForInput(campaign.date);
        document.getElementById('campaignCap').value = campaign.participantCap;
        document.getElementById('campaignStartTime').value = String(campaign.startTime || '').slice(0, 5);
        document.getElementById('campaignEndTime').value = String(campaign.endTime || '').slice(0, 5);
        document.getElementById('campaignOrganizer').value = campaign.organizerName || '';
        document.getElementById('campaignStatus').value = campaign.status;

        const preview = document.getElementById('campaignImagePreview');
        preview.innerHTML = campaign.imageUrl ? `<img src="${campaign.imageUrl}" alt="Current campaign image">` : '';
        preview.classList.toggle('has-image', Boolean(campaign.imageUrl));
        ...
        document.getElementById('cancelCampaignEdit').classList.remove('hidden');
        document.getElementById('adminCampaignForm').scrollIntoView({ behavior: 'smooth', block: 'start' });
      } catch (err) {}
    };

    window.deleteCampaign = async (campaignId) => {
      if (!confirm('Are you sure you want to delete this cleanup campaign?')) return;

      try {
        await apiCall(`/api/admin/campaigns/${campaignId}`, { method: 'DELETE' });
        showToast('Campaign deleted successfully', 'success');
        if (String(campaignId) === document.getElementById('campaignId').value) {
          resetCampaignForm();
        }
        loadAdminCampaigns();
      } catch (err) {}
    ...
    if (campaignForm) {
      setupPhotoPreview('campaignImage', 'campaignImagePreview', 'campaignUploadZone');
      ...
      campaignForm.addEventListener('submit', async (e) => {
        ...
        try {
          ...
          submitBtn.textContent = campaignId ? 'Updating...' : 'Launching...';
          await apiCall(campaignId ? `/api/admin/campaigns/${campaignId}` : '/api/admin/campaigns', {
            method: campaignId ? 'PUT' : 'POST',
            body: formData
          });
          showToast(campaignId ? 'Campaign updated successfully!' : 'Campaign launched successfully!', 'success');
          resetCampaignForm();
          loadAdminCampaigns();
        } catch (err) {
          submitBtn.textContent = campaignId ? 'Update Campaign' : 'Launch Campaign';
        } finally {
          submitBtn.disabled = false;
        }
      });

      document.getElementById('cancelCampaignEdit').addEventListener('click', resetCampaignForm);
    }

    const campaignSearchForm = document.getElementById('campaignSearchForm');
    if (campaignSearchForm) {
      campaignSearchForm.addEventListener('submit', (e) => {
        e.preventDefault();
        loadAdminCampaigns();
      });
    }

    document.querySelectorAll('#campaignStatusFilters .chip').forEach(chip => {
      chip.addEventListener('click', () => {
        document.querySelectorAll('#campaignStatusFilters .chip').forEach(item => item.classList.remove('active'));
        chip.classList.add('active');
        activeCampaignStatus = chip.dataset.status;
        loadAdminCampaigns();
      });
    });
